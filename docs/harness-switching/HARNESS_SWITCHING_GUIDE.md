...
RALPH_MODEL=gpt-4 RALPH_AGENT=coding ./scripts/ralph/ralph-story-run.sh --story path/to/story.json --dry-run 2>&1 | grep -E "\--model gpt\-4|\--agent coding"
```
     ...
     "tasks": [
       {
         ...
         "checks": [
           ...
           "grep -q 'Harness test successful' verification.txt"
         ],
         "depends_on": [],
         "status": "pending",
         "passes": false
