...
- Standalone PRD markdown generated by legacy `ralph-prd.sh` flows is committed when created.
- Epic PRD markdown generated from `promptContext` by legacy `ralph-prime.sh` flows is committed when created.
...
- Runtime artifacts are preserved by legacy archive flows under `scripts/ralph/tasks/archive/`.
...
[`scripts/ralph/ralph-roadmap.sh`](../scripts/ralph/ralph-roadmap.sh) now fills the missing top layer by:
...
The largest scoped-execution gap is now partially closed in [`scripts/ralph/ralph.sh`](../scripts/ralph/ralph.sh), and the core behavior is now backed by direct regression coverage.
...
- core Ralph lock semantics in `scripts/ralph/ralph.sh` and legacy `ralph-prime.sh` were not weakened
...
2. Run [`scripts/ralph/doctor.sh`](../scripts/ralph/doctor.sh)
3. Generate roadmap/sprint backlogs with [`scripts/ralph/ralph-roadmap.sh`](../scripts/ralph/ralph-roadmap.sh)
4. Review or activate sprint with [`scripts/ralph/ralph-sprint.sh`](../scripts/ralph/ralph-sprint.sh)
5. Refine epics manually only when needed

### Planning phase

Standalone path:

1. Run the legacy `ralph-prd.sh` flow
...
1. Normal path: run [`scripts/ralph/ralph.sh`](../scripts/ralph/ralph.sh); older revisions auto-called legacy `ralph-prime.sh` when needed
...
1. Run [`scripts/ralph/ralph.sh`](../scripts/ralph/ralph.sh)
2. Lock is acquired
...
1. Render `scripts/ralph/prompt.md` with live PRD/progress paths
2. Inject local prompt sections from `scripts/ralph/prompt.local.md` if present
...
1. Run the legacy `ralph-commit.sh` closeout flow
2. Validate all stories pass
3. Run the legacy archive flow
...
1. Run [`scripts/ralph/ralph-sprint-commit.sh`](../scripts/ralph/ralph-sprint-commit.sh)
...
- Roadmap-to-sprint planning now has a first-class implementation in [`scripts/ralph/ralph-roadmap.sh`](../scripts/ralph/ralph-roadmap.sh).
- Explicit file-scoped execution rules now have light runtime enforcement in [`scripts/ralph/ralph.sh`](../scripts/ralph/ralph.sh), with regression coverage for both allowed and blocked cases.
